...
// 参数说明     void
// 返回参数     void
// 使用示例     mpu6050_get_data();                                              // 执行该函数后，直接查看对应的变量即可
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void mpu6050_get_data (void)
{
    uint8 dat[14];  // 14字节：加速度(6) + 温度(2) + 陀螺仪(6)
    ...
    // dat[6]~dat[7] 是温度数据，如果不需要可以跳过
    
    // 解析陀螺仪数据
    mpu6050_gyro_x = (int16)(((uint16)dat[8]  << 8) | dat[9]);
    mpu6050_gyro_y = (int16)(((uint16)dat[10] << 8) | dat[11]);
    mpu6050_gyro_z = (int16)(((uint16)dat[12] << 8) | dat[13]);
}
